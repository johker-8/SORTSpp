...
:Radar scan:
   Manually constructed from :class:`~radar_scans.radar_scan` or a preset instance from :mod:`radar_scan_library`.
   The radar_scan instance should be set in the radar_system instance using the :func:`~radar_config.radar_system.set_scan` method.

:Scheduler instance:
   A scheduler instance is a function declaration located in :mod:`scheduler_library`. Different schedulers need
...
class ObservationParameters:
    ...
    def __init__(self,
                ...
                scan_during_interleaved = False,
            ):
        self.duty_cycle = duty_cycle
        self.SST_f = SST_f
        self.tracking_f = tracking_f
        self.coher_int_t = coher_int_t
        self.IPP = IPP
        self.interleaving_time_slice = interleaving_time_slice
        self.scan_during_interleaved = scan_during_interleaved

        self.calculate_parameters(
            duty_cycle=duty_cycle,
            SST_f=SST_f,
            tracking_f=tracking_f,
            coher_int_t=coher_int_t,
            IPP=IPP,
            interleaving_time_slice = interleaving_time_slice,
            scan_during_interleaved = scan_during_interleaved,
        )

    def calculate_parameters(self, **kwargs):

        duty_cycle = kwargs.setdefault('duty_cycle', self.duty_cycle)
        SST_f = kwargs.setdefault('SST_f', self.SST_f)
        tracking_f = kwargs.setdefault('tracking_f', self.tracking_f)
        coher_int_t = kwargs.setdefault('coher_int_t', self.coher_int_t)
        IPP = kwargs.setdefault('IPP', self.IPP)
        interleaving_time_slice = kwargs.setdefault('interleaving_time_slice', self.interleaving_time_slice)
        scan_during_interleaved = kwargs.setdefault('scan_during_interleaved', self.scan_during_interleaved)

        scanning_f = 1.0 - tracking_f
        track_duty = duty_cycle*tracking_f
        scan_duty = duty_cycle*scanning_f

        N_IPP = np.round(coher_int_t/IPP)
        coher_int_t = N_IPP*IPP

        T_pulse = IPP*duty_cycle
        T_pulse_track = IPP*track_duty
        T_pulse_scan = IPP*scan_duty

        if interleaving_time_slice == 0.0 or SST_f == 1.0:
            SST_slices = 1.0
            Interleaving_slices = 0.0
            SST_f_request = 1.0
        else:
            num = interleaving_time_slice*SST_f/(coher_int_t*(1.0-SST_f))
            if num > 1:
                num = n.round(num)
                SST_slices = num
                Interleaving_slices = 1.0
            else:
                num = n.round(1.0/num)
                SST_slices = 1.0
                Interleaving_slices = num
            SST_f_request = SST_f
            SST_f = SST_slices*coher_int_t/(SST_slices*coher_int_t + Interleaving_slices*interleaving_time_slice)

        self.N_IPP = N_IPP
        self.IPP = IPP
        self.T_pulse = T_pulse
        self.T_pulse_track = T_pulse_track
        self.T_pulse_scan = T_pulse_scan
        self.coher_int_t = coher_int_t
        self.interleaving_time_slice = interleaving_time_slice
        self.SST_slices = SST_slices
        self.Interleaving_slices = Interleaving_slices
        self.duty_cycle = duty_cycle
        self.SST_f = SST_f
        self.SST_f_request = SST_f_request
        self.tracking_f = tracking_f
        self.scanning_f = scanning_f
        self.scan_during_interleaved = scan_during_interleaved
    ...
    def save(self, fname):
        ...
        with h5py.File(fname,"w") as hf:
            hf.attrs['N_IPP'] = self.N_IPP
            hf.attrs['IPP'] = self.IPP
            hf.attrs['T_pulse'] = self.T_pulse
            hf.attrs['T_pulse_track'] = self.T_pulse_track
            hf.attrs['T_pulse_scan'] = self.T_pulse_scan
            hf.attrs['coher_int_t'] = self.coher_int_t
            hf.attrs['interleaving_time_slice'] = self.interleaving_time_slice
            hf.attrs['SST_slices'] = self.SST_slices
            hf.attrs['Interleaving_slices'] = self.Interleaving_slices
            hf.attrs['duty_cycle'] = self.duty_cycle
            hf.attrs['SST_f'] = self.SST_f
            hf.attrs['SST_f_request'] = self.SST_f_request
            hf.attrs['tracking_f'] = self.tracking_f
            hf.attrs['scanning_f'] = self.scanning_f
            hf.attrs['scan_during_interleaved'] = self.scan_during_interleaved
    ...
    def load(self, fname):
        ...
        with h5py.File(fname,"r") as hf:
            self.N_IPP = hf.attrs['N_IPP']
            self.IPP = hf.attrs['IPP']
            self.T_pulse = hf.attrs['T_pulse']
            self.T_pulse_track = hf.attrs['T_pulse_track']
            self.T_pulse_scan = hf.attrs['T_pulse_scan']
            self.coher_int_t = hf.attrs['coher_int_t']
            self.interleaving_time_slice = hf.attrs['interleaving_time_slice']
            self.SST_slices = hf.attrs['SST_slices']
            self.Interleaving_slices = hf.attrs['Interleaving_slices']
            self.duty_cycle = hf.attrs['duty_cycle']
            self.SST_f = hf.attrs['SST_f']
            self.SST_f_request = hf.attrs['SST_f_request']
            self.tracking_f = hf.attrs['tracking_f']
            self.scanning_f = hf.attrs['scanning_f']
            self.scan_during_interleaved = hf.attrs['scan_during_interleaved']


    def __str__(self):
        lines = []
        lines.append('<<< observation schedule >>>')
        lines.append('--> Duty cycle              : %.4f %%'%(self.duty_cycle*100.0,))
        lines.append('--> Requested SST allocation: %.4f %% of radar time'%(self.SST_f_request*100.0,))
        lines.append('--> Given SST allocation    : %.4f %% of radar time'%(self.SST_f*100.0,))
        lines.append('--> IPP                     : %.4f s'%(self.IPP,))
        lines.append('--> Tracking                : %.4f %%  = %.4f s / IPP'%(self.tracking_f*100.0,self.T_pulse_track))
        lines.append('--> Scaning                 : %.4f %%  = %.4f s / IPP'%(self.scanning_f*100.0,self.T_pulse_scan))
        lines.append('--> Tracking bandwidth      : %.4f Hz '%(1.0/(self.T_pulse_track*self.N_IPP),) )
        lines.append('--> Scaning  bandwidth      : %.4f Hz '%(1.0/(self.T_pulse_scan*self.N_IPP),) )
        lines.append('--> Total pulse length      : %.4f s / IPP'%(self.T_pulse))
        lines.append('--> Coherent pulse length   : %.4f s '%(self.T_pulse*self.N_IPP))
        lines.append('--> Coherent scan pulse     : %.4f s '% ( self.T_pulse_track*self.N_IPP,) )
        lines.append('--> Coherent track pulse    : %.4f s '% ( self.T_pulse_scan*self.N_IPP,) )
        lines.append('--> Coherent integration    : %i IPPs = %.4f s'%(self.N_IPP,self.coher_int_t))
        lines.append('--> SST time slices         : %i slices = %.4f s'%(self.SST_slices,self.SST_slices*self.coher_int_t))
        lines.append('--> Followed by')
        lines.append('--> Interleaved slices      : %i slices = %.4f s'%(self.Interleaving_slices,self.Interleaving_slices*self.interleaving_time_slice))
        lines.append('--> Interleaved data acsess : %r'%(self.scan_during_interleaved))
        return '\n'.join(lines)


    def configure_radar_to_observation(self, radar, mode=None):
        ...
        **Modes:**
        ...
        for tx in radar._tx:
            ...
            tx.n_ipp = self.N_IPP
            if mode=='scan':
                tx.pulse_length = self.T_pulse_scan
            elif mode=='track':
                tx.pulse_length = self.T_pulse_track
            else:
                tx.pulse_length = self.T_pulse

            tx.coh_int_bandwidth = 1.0/(tx.pulse_length*self.N_IPP)
            tx.duty_cycle = self.duty_cycle
